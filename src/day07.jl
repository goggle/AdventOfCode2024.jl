...
function solve(test_values, numbers)
    p1 = p2 = 0
    for (test_value, nl) ∈ zip(test_values, numbers)
        if valid_p1(test_value, nl[1], nl[2:end])
            p1 += test_value
            p2 += test_value
        elseif valid_p2(test_value, nl[1], nl[2:end])
            p2 += test_value
        end
    end
    return [p1, p2]
...
function valid_p1(goal, curr, remaining)
    isempty(remaining) && curr != goal && return false
    curr > goal && return false
    curr == goal && isempty(remaining) && return true
    return valid_p1(goal, curr + remaining[1], remaining[2:end]) || valid_p1(goal, curr * remaining[1], remaining[2:end])
end

function valid_p2(goal, curr, remaining)
    isempty(remaining) && curr != goal && return false
    curr > goal && return false
    curr == goal && isempty(remaining) && return true
    return valid_p2(goal, curr + remaining[1], remaining[2:end]) || valid_p2(goal, curr * remaining[1], remaining[2:end]) || valid_p2(goal, concat(curr, remaining[1]), remaining[2:end])
end
